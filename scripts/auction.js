...
const Dog = "0xa920464b46548930befecca5467860b2b4c2b5b9";

const contract = require("../artifacts/contracts/DogsAuctionHouse.sol/DogsAuctionHouse.json");
const contractAddress = "0xC9F32Fc6aa9F4D3d734B1b3feC739d55c2f1C1A7";
...
async function initialize() {
  ...
  const tx = {
    ...
    'nonce': nonce,
    'gas': 1000000,
    'data': nftContract.methods.initialize(Dog, WETH, 60*1, "100000000000000000", 10, 60*10).encodeABI()
  };
...
async function createBid(dogId, amount) {
  const nonce = await web3.eth.getTransactionCount(PUBLIC_KEY, 'latest'); //get latest nonce
  ...
  const tx = {
    ...
    'nonce': nonce,
    'gas': 1000000,
    'data': nftContract.methods.createBid(dogId, amount).encodeABI()
  };
...
async function bidToken() {
  const auction = await nftContract.methods.bidToken().call();
  console.log("The bidToken is: " + JSON.stringify(auction));
}
