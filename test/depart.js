...
const gasOptions = {"maxPriorityFeePerGas": "29000000000", "maxFeePerGas": "40000000016" };
...
describe("Degen Dog Polygon Departure", function () {


  describe("Governance", function () {

    before("load gov proxy with implementation abi", async function(){
      //console.log("chain", await hre.getChainId());
    ...
    it('Should create proposal with 2 txns', async function () {
      ...
      // Txn 1: Redeem idleWETH to WETH
      targets.push(addr.idleWETH);
      values.push(0);
      signatures.push('redeemIdleToken(uint256)');
      callDatas.push(encodeParameters(['uint256'], ['345768715059780500']));

      // Txn 2: Send WETH to dogBot address
      const weth = new ethers.Contract(addr.WETH, ERC20abi, signer);
      const balance = await weth.balanceOf(addr.treasury);
      console.log("balance", balance.toString());
      targets.push(addr.WETH);
      values.push(0);
      signatures.push('transfer(address,uint256)');
      callDatas.push(encodeParameters(['address','uint256'], [addr.dogBot, balance.toString()]));

      var description = "1. Redeem idleWETH to WETH 2. Send WETH to dogBot";
    ...
    it.skip('Should have dogBot WETH balance', async function () {
      this.timeout(2400000);
      const weth = new ethers.Contract(addr.WETH, ERC20abi, signer);
      const balance = await weth.balanceOf(addr.dogBot);
      console.log("balance", balance.toString());
